...
<body>
    ...
    <?php $strc_keys = ['product_dates' => 'Datos del producto', 'product_name' => 'Nombre del producto', 'lot' => 'Lote No.', 'dilution' => 'Dilución', 'injectable' => 'Concentración de inyectable', 'total' => 'Total unidades administradas', 'complications' => 'Complicaciones', 'record_complications' => 'Registro de complicaciones', 'participants' => 'Participantes', 'comments' => 'Comentarios']; ?>
    <?php $strc_item = ['muscle' => 'Músculo', 'units' => 'Unidades administradas']; ?>
    @if (count($o_hcpro) > 0)
        <ul>
            @foreach ($o_hcpro as $item)
                <li>
                    @foreach ($strc_keys as $stkey => $strow)
                        ...
                        @endisset
                    @endforeach
                    @if (count($item->htreatment) > 0)
                        <b>Tratamientos:</b>
                        <ul>
                            @foreach ($item->htreatment as $treat)
                                <li>
                                    @foreach ($strc_item as $key => $row)
                                        @isset($treat->{$key})
                                            <div>
                                                <b>{{ $row }}:</b>{{ $treat->{$key} }}
                                            </div>
