...
<body>
    ...
    <?php $strc_keys = ['disinfection' => 'Desinfección', 'antiseptic' => 'Cual antiséptico?', 'anesthesia' => 'Anestésia', 'type_anesthesia' => 'Tipo de anestésia', 'other_anesthesia' => 'Cual?', 'hemostasis' => 'Hemostasia', 'other_hemostasis' => 'Cual?', 'procedure_time' => 'Tiempo del procedimiento', 'complications' => 'Complicaciones', 'record_complications' => 'Registro de complicaciones presentadas', 'participants' => 'Datos de quienes participaron en el procedimiento', 'comments' => 'Comentarios adicionales', 'other_procedure' => 'Cual?', 'biopsy_method' => 'Método de biopsia', 'other_biopsy_method' => 'Cual?', 'biopsy_type' => 'Tipo de biopsia', 'required_margin' => 'Requirió margen', 'how_much' => 'Cuanto?', 'body_area' => 'Área', 'body_area_other' => 'Área corporal']; ?>
    @if (count($o_hcpro) > 0)
        <ul>
            @foreach ($o_hcpro as $item)
                <li>
                    ...
                    @foreach ($strc_keys as $stkey => $strow)
                        ...
                        @endisset
                    @endforeach
                    @if (count($item->hcsuture) > 0)
                        <b>Suturas</b>
                        <ul>
                            @foreach ($item->hcsuture as $suture)
                                <li>
                                    <p><b>Tipo de sutura:</b> {{ $suture->suture_type }}</p>
                                    <p><b>Calibre:</b> {{ $suture->caliber }}</p>
                                </li>
                            @endforeach
                        </ul>
                    @endif
