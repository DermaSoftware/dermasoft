...
@section('content')
                    ...
                    <div class="right">
                        <a href="<?= url($menu . '/create') ?>"
                           class="button h-button is-success is-rounded is-elevated m-r-10"><span class="icon"><i
                                    class="fas fa-plus"></i></span> <span>Crear <?= $c_name ?></span></a>
    ...
    <div class="s-card">
        <table class="o_stable_fsc table display compact is-fullwidth" style="width:100%">
            ...
            <tr id="<?= $row->uuid ?>" <?= $row->plan_id === 4 ? "style=background:#d1e2de" : ""?>>
                <td><?= $key + 1 ?></td>
                            ...
                            <div class="dropdown-menu" role="menu">
                                <div class="dropdown-content">
                                    <a href="<?= url($menu . '/' . $row->uuid) ?>" class="dropdown-item is-media">
                                        ...
                                        <div class="meta">
                                            <span>Ver</span>
                                            <span>Detalles  <?= $tag_the ?> <?= $c_name ?></span>
                                        </div>
                                    </a>
                                    ...
                                    <a id="<?= $row->uuid ?>" href="javascript:void(0)"
                                       data-href="<?= url($menu . '/' . $row->uuid) ?>" data-itemtag="<?= $tag_the ?>"
                                       data-itemselect="<?= $tag_o ?>" data-nameitem="<?= $c_name ?>"
    ...
    <div class="s-card">
        <fieldset>
            ...
            <div style="display: flex" class="mt-5">
                <div style="background:#d1e2de;width: 20px;height: 20px;"></div>
                <div class="pl-2">Compañia con membresia de prueba</div>
            </div>
        </fieldset>
    </div>
@endsection
