...
use App\Models\Examorders;
use App\Models\Eoexams;
use App\Models\Eodiagnostics;
use PDF;
...
class ExamordersController extends Controller
{
    ...
    private $c_name = 'Órden de exámenes';
    private $c_names = 'Órdenes de exámenes';
	...
	public function store(Request $request, $id)
    {
		...
		$validatedData = $request->validate([
			'solexams_exam' => 'required',
		],[
			'solexams_exam.required' => 'La solicitud de examenes es requerida',
		]);
		if(empty($id)){
			return redirect($this->r_name);
		}
		...
		if(!empty($data['diagnostics'])){
			foreach($data['diagnostics'] as $key => $row){
				...
				$o_x = Eodiagnostics::create($aux_params);
			}
		}
		...
		if(!empty($data['notification_email']) AND $data['notification_email'] == 'yes'){
			...
			Mail::to($ox->email)->send(new ExamordersMail($full_name,$attach_file));
		}
