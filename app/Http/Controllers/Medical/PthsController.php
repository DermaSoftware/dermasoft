...
class PthsController extends Controller
{
    ...
    private $c_name = 'Solicitud de patologías';
    private $c_names = 'Solicitudes de patologías';
	...
	public function store(Request $request, $id)
    {
		...
		if(!empty($data['code'])){
			foreach($data['code'] as $key => $row){
				$aux_params = ['pt' => $o->id];
				$aux_params['code'] = !empty($row)?$row:'';//
				$aux_params['pathologie'] = !empty($data['pathologie'][$key])?$data['pathologie'][$key]:'';//
		...
		if(!empty($data['notification_email']) AND $data['notification_email'] == 'yes'){
			...
			Mail::to($ox->email)->send(new PthsMail($full_name,$attach_file));
		}
