...
class TrainingsController extends Controller
{
    private $tag_the = 'El';
    private $tag_o = 'o';
    private $r_name = 'admin/trainings';
    private $v_name = 'admin.trainings';
    ...
    {
        $data = $this->gdata();
        $data['o_all'] = $this->o_model::where(['company' => 0])->whereNotIn('status', ['deleted'])->orderBy('id', 'DESC')->get();
    ...
    public function store(Request $request)
    {
        ...
        //https://www.youtube.com/embed/lkupUddL4ww
        //https://www.youtube.com/embed/SX6dWnfKky8
    ...
    public function hidviews()
    {
        ...
        $o_all = $local_model::where(['status' => 'active'])->orderBy('id', 'asc')->get();
    ...
    public function resendpbr($id)
    {
        ...
        if ($o->directed_to == 'Todos') {
            $o_all = User::where(['status' => 'active'])->whereNotIn('status', ['deleted'])->orderBy('id', 'asc')->get();
        } else if ($o->directed_to == 'Roles') {
            $ids = Trainingsroles::where(['trainings_id' => $o->id])->pluck('roles_id')->toArray();
            $o_all = User::where(['status' => 'active'])->whereIn('role', $ids)->orderBy('id', 'asc')->get();
        } else if ($o->directed_to == 'Users') {
            $ids = Trainingsusers::where(['trainings_id' => $o->id])->pluck('users_id')->toArray();
            $o_all = User::where(['status' => 'active'])->whereIn('id', $ids)->orderBy('id', 'asc')->get();
        }
        foreach ($o_all as $key => $row) {
            Mail::to($row->email)->send(new Ntfs('Nueva capacitación',
                'Enhorabuena!!! Nos complace notificarte que se ha publicado la capacitación ' . $o->name . ',
                 para visualizar el contenido te invitamos a ingresar a nuestra plataforma y
                 acceder al módulo de capacitaciones.', $row->name, $row->email));
        }
