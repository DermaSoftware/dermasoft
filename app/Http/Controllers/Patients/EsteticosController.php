...
class EsteticosController extends Controller
{
	private $tag_the = 'El';
    private $tag_o = 'o';
    private $r_name = 'esteticos';
	...
	private function getpdfhc($id, $save = false)
    {
		...
		$pdf = PDF::loadView('pdf.aesthetic', $data);
	...
	public function records()
    {
		...
		$pdfFilePath = $this->allpdfhc($o->id);
    }
	private function allpdfhc($id)
    {
		...
		$o = $this->o_model::where(['id' => $id])->first();
		$full_name = $o->name.' '.$o->scd_name.' '.$o->lastname.' '.$o->scd_lastname;
		...
		$data['arr'] = $arr;
		$pdf = PDF::loadView('pdf.aestheticall', $data);
		return $pdf->stream('document.pdf');
		exit();
    }
}
