...
class ProcedimientosController extends Controller
{
	...
	public function hcpdf($id)
    {
		...
		$o_obj_item = ProcedureRequest::where(['uuid' => $id])->first();
	...
	private function allpdfhc($id)
    {
		...
		$arr = [];
        $hc = $o->lastHc;
        $derm_all = ProcedureRequest::with([
            'dermatology',
            'doctor_class',
            'procedures'
        ])
        ->where(['dermatology_id' => $hc->id])->get(['uuid','id','dermatology_id','doctor','created_at']);
		...
		foreach($derm_all as $key => $o_obj_item){
			...
			$dfull_name = $o_doctor->name.' '.$o_doctor->scd_name.' '.$o_doctor->lastname.' '.$o_doctor->scd_lastname;
			$signature = !empty($o_doctor->signature_pp)?$o_doctor->signature_pp:public_path('assets/images/firma.png');
			$all_items = $o_obj_item->procedures; //Prodsitem::where(['pd' => $o_obj_item->id])->orderBy('id', 'asc')->get();//Items
