...
use App\Models\Hcaesthetic;
use App\Models\Hctreatment;
...
class AestheticController extends Controller
{
    private $tag_the = 'El';
    private $tag_o = 'o';
    ...
    private $hc_view = 'aesthetic';
    private $hc_type = 'Procedimientos Estéticos';
    ...
    public function store(Request $request, $id)
    {
        ...
        $all_params = ['product_dates', 'product_name', 'lot', 'dilution', 'injectable', 'total', 'complications', 'record_complications', 'participants', 'comments'];
        unset($all_params['total']);
        foreach ($all_params as $key => $row) {
            $aux_params[$row] = !empty($data[$row]) ? $data[$row] : '';
        }
        $total = 0;
        $all_params['total'] = 0;
        $o_hcpro = Hcaesthetic::create($aux_params);
        //HCTREATMENT
        if (!empty($data['muscle'])) {
            foreach ($data['muscle'] as $key => $row) {
                ...
                $total += $units;
                $o_x = Hctreatment::create($aux_params);
            }
    ...
    private function getpdfhc($id, $save = false)
    {
        ...
        // $all_sut = Hctreatment::where(['hc' => $o_derm->id])->orderBy('id', 'asc')->get(); //Suture
    ...
    private function allpdfhc($id)
    {
        ...
        foreach ($derm_all as $key => $o_derm) {
            ...
            $dfull_name = $o_doctor->name . ' ' . $o_doctor->scd_name . ' ' . $o_doctor->lastname . ' ' . $o_doctor->scd_lastname;
            $signature = !empty($o_doctor->signature_pp) ? $o_doctor->signature_pp : public_path('assets/images/firma.png');
            $o_hcpro = Hcaesthetic::where(['hc' => $o_derm->id])->first();
            $all_sut = Hctreatment::where(['hc' => $o_derm->id])->orderBy('id', 'asc')->get(); //Suture
            $all_dgs = Hcdermdiagnostics::where(['hc' => $o_derm->id])->orderBy('id', 'asc')->get(); //diagnosticos
            $all_ind = Hcdermindications::where(['hc' => $o_derm->id])->orderBy('id', 'asc')->get(); //indicaciones
            $all_pre = Prescriptions::where(['hc' => $o_derm->id])->orderBy('id', 'asc')->get(); //prescripción médica
